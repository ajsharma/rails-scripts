      ...
      class << self
        ...
        def run
          RailsScripts::System.call <<~SH
            git diff --relative --name-only $(git merge-base #{RailsScripts.configuration.git_trunk_branch_name} HEAD) | grep -e ".rb$" | xargs -t bundle exec bin/yard doc --no-cache --fail-on-warning
          SH
        end
      end
    end
